...
class Utils(object):
    ...
    def two_d_stencil(self, num, epsilon=0.0, epsilon_sparse=False, grid_size=8):
        if self.bc == 'dirichlet':
            ...
            return self.two_d_stencil_periodicBC(num=num, epsilon=epsilon, epsilon_sparse=epsilon_sparse,
                                                 grid_size=grid_size)
    ...
    def solve_blackbox(self, A_matrices, b, initial_guess, max_iterations, max_depth=3, w_cycle=False, periodic=True):
        def prolongation_fn(A, args):
            ...
            with tf.device(self.device):
                model = blackbox.black_box_dirichlet_output_transform if not periodic else blackbox.black_box_periodic_output_transform
                P_stencil = model(inputs=tf_A_stencil, grid_size=grid_size)
        ...
        solver = geometric_solver(A_matrices, prolongation_fn, {}, max_levels=max_depth)
    ...
    def solve_with_model(self, model: PNetwork, A_matrices, b, initial_guess, input_transforms, output_transforms,
        ...
        def prolongation_fn(A, args):
            ...
            output_transforms = args["output_transforms"]
            grid_size = int(math.sqrt(A.shape[0]))
            indices = self.get_indices_compute_A_one(grid_size)
            A_stencil = np.array(A[indices[:, 0], indices[:, 1]]).reshape((grid_size, grid_size, 3, 3))
            tf_A_stencil = tf.convert_to_tensor([A_stencil])
            ...
            with tf.device(self.device):
                transformed_inputs = input_transforms(tf_A_stencil, grid_size)
                prediction = model(inputs=transformed_inputs, training=False)
                P_stencil = output_transforms(tf_A_stencil, grid_size, prediction)

            return self.compute_p2(P_stencil, grid_size).astype(np.double)  # imaginary part should be zero

        prolongation_args = {"input_transforms": input_transforms,
